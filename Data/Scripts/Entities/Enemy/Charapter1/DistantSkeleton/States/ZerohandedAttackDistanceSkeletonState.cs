using Godot;
using System;

public partial class ZerohandedAttackDistanceSkeletonState : Node, IDistantSkeletonState
{
    private DistantSkeleton _enemy;
    private Timer _endTimer = new Timer { Autostart = true, OneShot = true, WaitTime = 3 };
    private Timer _attackTimer = new Timer { Autostart = true, OneShot = true, WaitTime = 2 };

    public ZerohandedAttackDistanceSkeletonState(DistantSkeleton enemy)
    {
        _enemy = enemy;
        AddChild(_attackTimer);
        AddChild(_endTimer);
        _attackTimer.Timeout += () => _enemy.Attack(new DistanSkeletonThrowHandAttack(_enemy.Damage, 2, _enemy.GlobalPosition, Global.SceneObjects.Player?.GlobalPosition ?? Vector2.Zero));
        _endTimer.Timeout += () => _enemy.TakeDamage(_enemy.Health);
    }

    public string GetAnimation() =>
        "zerohanded_attack";
}
